...
class CourseEnrollmentRepository(BaseRepository[CourseEnrollment]):
    ...
    def get_by_student_and_course(self, student_id: str, course_id: str) -> CourseEnrollment | None:
    ...
    def list_filtered(self, student_id: str | None = None) -> list[CourseEnrollment]:
        stmt = select(CourseEnrollment).where(CourseEnrollment.deleted_at.is_(None))
        if student_id:
            stmt = stmt.where(CourseEnrollment.student_id == student_id)
