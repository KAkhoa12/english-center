...
@router.get("/my")
def my_enrollments(db: Annotated[Session, Depends(get_db)], current_user: User = Depends(require_permission("order.read")), page: int = Query(1), page_size: int = Query(10)):
