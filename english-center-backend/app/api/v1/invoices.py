...
from app.services.commerce_service import OrderSerializer
...
def list_invoices(
    ...
    items, total = OrderSerializer(db).list_invoices(query, current_user, status, user_id)
    return api_response(True, "Invoices retrieved successfully", items, build_pagination(page, page_size, total))
...
def my_invoices(db: Annotated[Session, Depends(get_db)], current_user: User = Depends(require_permission("invoice.read")), page: int = Query(1), page_size: int = Query(10)):
    query = PaginationParams(page=page, page_size=page_size)
    items, total = OrderSerializer(db).list_invoices(query, current_user)
    return api_response(True, "My invoices retrieved successfully", items, build_pagination(page, page_size, total))
...
def get_invoice(invoice_id: str, db: Annotated[Session, Depends(get_db)], current_user: User = Depends(require_permission("invoice.read"))):
    data = OrderSerializer(db).get_invoice(invoice_id, current_user)
    return api_response(True, "Invoice retrieved successfully", data, None)
...
def get_order_invoice(order_id: str, db: Annotated[Session, Depends(get_db)], current_user: User = Depends(require_permission("invoice.read"))):
    data = OrderSerializer(db).get_order_invoice(order_id, current_user)
    return api_response(True, "Invoice retrieved successfully", data, None)
