...
type TeacherFormProps = {
  ...
  loading?: boolean;
  isCreate?: boolean;
  onSubmit: (payload: TeacherCreateRequest | TeacherUpdateRequest, avatarFile?: File | null) => Promise<void>;
};
    ...
    <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
        <div className="rounded-2xl bg-gray-50 p-4">
          ...
          <p className="mt-1 text-xs text-gray-500">Dùng ảnh rõ mặt để học viên nhận diện giáo viên.</p>
          <div className="mt-4 flex flex-col items-center gap-3">
            {avatarPreview || currentAvatar ? (
              <img src={avatarPreview || currentAvatar || ""} alt="Avatar giáo viên" className="h-28 w-28 rounded-2xl object-cover ring-1 ring-gray-200" />
            ) : (
              <div className="flex h-28 w-28 items-center justify-center rounded-2xl bg-blue-50 text-2xl font-bold text-blue-700 ring-1 ring-blue-100">
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-2xl border border-gray-100 p-4">
            <h3 className="text-base font-semibold text-gray-900">Thông tin giáo viên</h3>
            <div className="mt-4 space-y-4">
              <Field label="Chuyên môn">
                <Input value={specialization} onChange={(event) => setSpecialization(event.target.value)} placeholder="VD: IELTS Speaking, TOEIC Listening" />
              </Field>
              <div className="space-y-1.5">
                <FieldLabel>Giới thiệu</FieldLabel>
                <Textarea value={bio} onChange={(event) => setBio(event.target.value)} placeholder="Kinh nghiệm, phong cách giảng dạy, chứng chỉ nổi bật" rows={5} />
              </div>
            </div>
          </section>
        </div>
