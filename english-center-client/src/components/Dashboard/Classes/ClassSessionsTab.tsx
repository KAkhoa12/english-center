...
import { DashboardConfirmDeleteDialog, DashboardDateInput } from "@/components/Dashboard/Comon";
import { Button } from "@/components/ui/button";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useClassSessionsStore } from "@/services/classSessions/classSessions.store";
import type { BulkCreateClassSessionsRequest, ClassSession, CreateClassSessionRequest, UpdateClassSessionRequest } from "@/services/classSessions/classSessions.type";
...
export function ClassSessionsTab({ classItem }: ClassSessionsTabProps) {
  const { sessions, pagination, isLoading, listAllSessions, createSession, createSessionsBulk, updateSession, deleteSession } = useClassSessionsStore();
  ...
  useEffect(() => {
    ...
    void listLessons(classItem.course_id, { page: 1, page_size: 100, status: "active" }).catch(() => toast.error("Không thể tải bài học"));
  ...
  useEffect(() => {
    ...
    }).catch((error) => toast.error(error instanceof Error ? error.message : "Không thể tải lịch học"));
  }, [classItem.id, listAllSessions, page, pageSize, status, mode, fromDate, toDate]);
  ...
  const refresh = () => listAllSessions({ page, page_size: pageSize, class_id: classItem.id, status: status === "all" ? undefined : status, mode: mode === "all" ? undefined : mode, from_date: fromDate || undefined, to_date: toDate || undefined });

  const handleSubmit = async (data: CreateClassSessionRequest | UpdateClassSessionRequest) => {
    try {
      if (editing) {
  ...
  const handleCreateBulk = async (data: BulkCreateClassSessionsRequest) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Tạo lịch học tự động thất bại");
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-gray-100 bg-white p-4">
        <Select value={status} onValueChange={(value) => { setStatus(value); setPage(1); }}><SelectTrigger className="w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả trạng thái</SelectItem>{sessionStatusOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select>
        <Select value={mode} onValueChange={(value) => { setMode(value); setPage(1); }}><SelectTrigger className="w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả hình thức</SelectItem>{sessionModeOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select>
        <DashboardDateInput value={fromDate} onChange={(value) => { setFromDate(value); setPage(1); }} className="w-40" />
        <DashboardDateInput value={toDate} onChange={(value) => { setToDate(value); setPage(1); }} className="w-40" />
        <Button type="button" onClick={() => { setEditing(null); setDialogOpen(true); }}>Thêm lịch học mới</Button>
