...
export function ClassSessionDialog({ open, session, schedules, teacherOptions, lessonOptions, roomOptions, onOpenChange, onSubmit }: ClassSessionDialogProps) {
  ...
  const handleSubmit = async () => {
    const overridePayload = form.overrideStartTime && form.overrideEndTime
      ? { override_start_time: form.overrideStartTime, override_end_time: form.overrideEndTime }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <label className="space-y-2 text-sm font-medium text-gray-700">Lịch học trong tuần<Select value={form.classScheduleId} onValueChange={(classScheduleId) => update({ classScheduleId })}><SelectTrigger><SelectValue placeholder="Chọn lịch học" /></SelectTrigger><SelectContent>{schedules.map((item) => <SelectItem key={item.id} value={item.id}>{item.schedule_name} · {timeValue(item.start_time)} - {timeValue(item.end_time)}</SelectItem>)}</SelectContent></Select></label>
          ...
          <div className="grid grid-cols-2 gap-3 md:col-span-2">
            <label className="space-y-2 text-sm font-medium text-gray-700">Override bắt đầu<Input type="time" value={form.overrideStartTime} onChange={(event) => update({ overrideStartTime: event.target.value })} /></label>
            <label className="space-y-2 text-sm font-medium text-gray-700">Override kết thúc<Input type="time" value={form.overrideEndTime} onChange={(event) => update({ overrideEndTime: event.target.value })} /></label>
          </div>
        ...
        <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Hủy</Button><Button onClick={() => void handleSubmit()} disabled={!form.classScheduleId}>{isEdit ? "Lưu cập nhật" : "Thêm lịch học"}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
