...
import { DashboardDateInput, DashboardListPageHeader, SectionCard } from "@/components/Dashboard/Comon";
...
export default function DashboardSessionDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<(typeof tabs)[number]["value"]>("info");
  ...
  useEffect(() => {
    if (!sessionId) return;
    void getSession(sessionId).catch((error) => toast.error(error instanceof Error ? error.message : "Không thể tải buổi học"));
  ...
  useEffect(() => {
    ...
    void listTeachers({ page: 1, page_size: 100 }).catch(() => toast.error("Không thể tải giáo viên"));
    void listRooms({ page: 1, page_size: 100, status: "active" }).catch(() => toast.error("Không thể tải phòng học"));
  }, [activeTab, listRooms, listTeachers]);

  useEffect(() => {
    if (activeTab !== "info" || !selectedSession?.course?.id) return;
    void listLessons(selectedSession.course.id, { page: 1, page_size: 100, status: "active" }).catch(() => toast.error("Không thể tải bài học"));
  }, [activeTab, listLessons, selectedSession?.course?.id]);

  useEffect(() => {
    if (!sessionId || activeTab !== "attendance") return;
    void getSessionAttendance(sessionId, { page: 1, page_size: 100 }).catch((error) => toast.error(error instanceof Error ? error.message : "Không thể tải điểm danh"));
  }, [activeTab, getSessionAttendance, sessionId]);

  useEffect(() => {
    if (!sessionId || activeTab !== "materials") return;
    void listMedia(sessionId).catch((error) => toast.error(error instanceof Error ? error.message : "Không thể tải tài liệu"));
  ...
  const handleAttendanceStatus = async (item: AttendanceItem, status: string) => {
    if (!sessionId || status === item.status) return;
    try {
      if (item.id) {
        await updateAttendance(item.id, { status });
      } else {
        await markAttendance(sessionId, [{ student_id: item.student_id, status }]);
      }
      await refreshAttendance();
      toast.success("Đã cập nhật điểm danh");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Cập nhật điểm danh thất bại");
    }
  };
  ...
  const handleSaveInfo = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Cập nhật buổi học thất bại");
    }
  };
  ...
  if (!selectedSession) {
    return (
      <section>
        <DashboardListPageHeader title="Chi tiết buổi học" description="Đang tải dữ liệu buổi học" />
  ...
  return (
    <section>
      <DashboardListPageHeader
        ...
        actions={(
          <Button type="button" variant="outline" onClick={() => navigate(PRIVATE_ROUTES.DASHBOARD_SESSIONS)}>
            Quay lại
          </Button>
        )}
      />
          ...
          {infoForm ? (
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <InfoRow label="Khóa học" value={selectedSession.course?.name} />
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm font-medium text-gray-700">Tên buổi học<Input value={infoForm.title} onChange={(event) => updateInfoForm({ title: event.target.value })} /></label>
                <label className="space-y-2 text-sm font-medium text-gray-700">Giáo viên<SearchableSelect value={infoForm.teacherId} options={teacherOptions} placeholder="Chọn giáo viên" onChange={(teacherId) => updateInfoForm({ teacherId })} /></label>
                <label className="space-y-2 text-sm font-medium text-gray-700">Bài học<SearchableSelect value={infoForm.lessonId} options={lessonOptions} placeholder="Chọn bài học" onChange={(lessonId) => updateInfoForm({ lessonId })} /></label>
                <label className="space-y-2 text-sm font-medium text-gray-700">Hình thức<Select value={infoForm.mode} onValueChange={(mode) => updateInfoForm({ mode })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{sessionModeOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></label>
                {infoForm.mode === "offline" ? (
                  <label className="space-y-2 text-sm font-medium text-gray-700">Phòng học<SearchableSelect value={infoForm.roomId} options={roomOptions} placeholder="Chọn phòng học" onChange={(roomId) => updateInfoForm({ roomId })} /></label>
                ) : (
                  <label className="space-y-2 text-sm font-medium text-gray-700">Link học online<Input value={infoForm.meetingUrl} onChange={(event) => updateInfoForm({ meetingUrl: event.target.value })} placeholder="Có thể để trống" /></label>
                )}
                ...
                <label className="space-y-2 text-sm font-medium text-gray-700 md:col-span-2">Ghi chú<Textarea value={infoForm.note} onChange={(event) => updateInfoForm({ note: event.target.value })} /></label>
              </div>
              <div className="flex justify-end">
                <Button type="button" onClick={() => void handleSaveInfo()}>Lưu thông tin</Button>
              </div>
            </div>
          ) : null}
        </SectionCard>
        ...
        <SectionCard title="Điểm danh học viên">
          <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white">
            <Table>
              ...
              <TableBody>
                {attendance.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="py-8 text-center text-gray-500">Chưa có học viên để điểm danh</TableCell></TableRow>
                ) : attendance.map((item) => (
                  <TableRow key={`${item.student_id}-${item.id ?? "empty"}`}>
                    <TableCell className="font-semibold text-gray-900">{item.student.full_name}</TableCell>
                    <TableCell>{item.student.email}</TableCell>
                    <TableCell>
                      <Select value={item.status} onValueChange={(status) => void handleAttendanceStatus(item, status)}>
                        <SelectTrigger className="w-40"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {attendanceStatusOptions.map((status) => <SelectItem key={status.value} value={status.value}>{status.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </TableCell>
        ...
        <SectionCard title="Tài liệu tham khảo">
          <div className="mb-5 rounded-2xl border border-gray-100 bg-gray-50 p-4">
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-2 text-sm font-medium text-gray-700">File tài liệu<Input type="file" onChange={(event) => setUploadFile(event.target.files?.[0] ?? null)} /></label>
              <label className="space-y-2 text-sm font-medium text-gray-700">Tiêu đề<Input value={uploadTitle} onChange={(event) => setUploadTitle(event.target.value)} placeholder={uploadFile?.name ?? "Tên tài liệu"} /></label>
              <label className="space-y-2 text-sm font-medium text-gray-700 md:col-span-2">Mô tả<Textarea value={uploadDescription} onChange={(event) => setUploadDescription(event.target.value)} /></label>
            </div>
            <div className="mt-3 flex justify-end">
              <Button type="button" onClick={() => void handleUploadMaterial()}>Upload tài liệu</Button>
            </div>
          </div>
              ...
              {sessionMedia.map((item) => (
                <div key={item.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-gray-900">{item.title || item.media?.original_filename || "Tài liệu buổi học"}</p>
                      <p className="mt-1 text-sm text-gray-500">{item.description || "Không có mô tả"}</p>
                    </div>
                    {item.media?.url ? (
                    ...
                    <Button type="button" variant="destructive" onClick={() => void handleDeleteMaterial(item.id)}>
                      Xóa
                    </Button>
                  </div>
                </div>
              ...
              {assignments.map((assignment) => (
                <div key={assignment.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                  ...
                  <div className="mt-3 grid gap-2 text-sm text-gray-500 md:grid-cols-3">
                    ...
                    <span>Hạn nộp: {assignment.due_at ? new Date(assignment.due_at).toLocaleString("vi-VN") : "Không giới hạn"}</span>
                  </div>
                </div>
              ))}
            </div>
