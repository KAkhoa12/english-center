...
import type { StaffUpdateRequest } from "@/services/staff/staff.type";
import { PRIVATE_ROUTES } from "@/shared/routes";

export default function DashboardStaffEditPage() {
  const { staffId = "" } = useParams();
  const navigate = useNavigate();
  const { selectedStaff, isLoading, getStaff, updateStaff, updateStaffAvatar, clearSelectedStaff } = useStaffStore();
  ...
  return (
    <section>
      <DashboardListPageHeader title="Chỉnh sửa nhân viên" description="Cập nhật thông tin nhân viên" />
      <StaffForm
        initialData={selectedStaff}
        loading={isLoading}
        onSubmit={async (payload, avatarFile) => {
          try {
            await updateStaff(staffId, payload as StaffUpdateRequest);
            if (avatarFile) await updateStaffAvatar(staffId, avatarFile);
            toast.success("Cập nhật nhân viên thành công");
            navigate(PRIVATE_ROUTES.DASHBOARD_STAFF);
          } catch {
