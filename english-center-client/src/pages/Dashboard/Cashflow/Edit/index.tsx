...
export default function DashboardCashflowEditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const item = await cashflowMockStore.getById(cashflowId);
      if (!item) {
        toast.error("Khong tim thay ban ghi thu chi");
    ...
    if (cashflowId) void load();
  }, [cashflowId, navigate]);

  const handleSubmit = async () => {
    try {
      await cashflowMockStore.update(cashflowId, { title, type, amount, transaction_date: transactionDate, note });
      toast.success("Cap nhat thu chi thanh cong");
      navigate(PRIVATE_ROUTES.DASHBOARD_FINANCE_CASHFLOW);
    } catch {
      toast.error("Cap nhat thu chi that bai");
    }
  };

  return (
    <section>
      <DashboardListPageHeader title="Chinh sua thu chi" description="Trang sua (UI mock, chua ket noi API)" />
